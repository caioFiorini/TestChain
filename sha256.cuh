...
namespace SHA256CUDA
{
    ...
    __device__ char *sha256(const char *input)
    {
        ...
        SHA256 ctx;
        ctx.init();
        ctx.update(reinterpret_cast<const unsigned char *>(input), count);
        ctx.final(digest);

        // Aloca memória para o resultado hexadecimal no dispositivo
        char *output = new char[2 * SHA256::DIGEST_SIZE + 1];
        ...
        output[2 * SHA256::DIGEST_SIZE] = '\0'; // Termina a string com null
